...
static int help(void) {
        printf("%s [OPTIONS...]\n\n"
               ...
               "  set-dynamic                  dev [DEVICE] dhcp [DHCP {BOOLEAN|ipv4|ipv6}] use-dns-ipv4 [BOOLEAN] use-dns-ipv6 [BOOLEAN] send-release-ipv4 [BOOLEAN] send-release-ipv6 [BOOLEAN]"
                                                      "\n\t\t\t\t use-domains-ipv4 [BOOLEAN] use-domains-ipv6 [BOOLEAN] accept-ra [BOOLEAN] dhcp4-client-id [IDENTIFIER {mac|duid|duid-only}"
               "                                       \n\t\t\t\t keep [BOOLEAN] Configures dynamic configuration of the device (IPv4|IPv6|RA).\n"
