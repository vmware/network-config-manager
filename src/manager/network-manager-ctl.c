...
static int help(void) {
        printf("%s [OPTIONS...]\n\n"
               ...
               "  add-additional-gw            dev [DEVICE] address|addr|a [ADDRESS] destination|dest [DESTINATION address] gw [GW address] table [TABLE NUMBER] Configures additional"
               ...
               "  add-ntp                      dev [DEVICE] [NTP] Add Link NTP server address. This option may be specified more than once.\n"
               "                                      This setting is read by systemd-timesyncd.service(8)\n"
               "  set-ntp                      dev [DEVICE] [NTP] Set Link NTP server address. This option may be specified more than once.\n"
               "                                      This setting is read by systemd-timesyncd.service(8).\n"
               ...
               "  add-sr-iov                   dev [DEVICE] [vf INTEGER] [vlanid INTEGER] [qos INTEGER] [vlanproto STRING] [macspoofck BOOLEAN] [qrss BOOLEAN] [trust BOOLEAN] [linkstate BOOLEAN or STRING] [macaddr ADDRESS] Configures SR-IOV VirtualFunction, VLANId, QualityOfService, VLANProtocol, MACSpoofCheck, QueryReceiveSideScaling, Trust, LinkState, MACAddress  managed by networkd.\n"
               "  create-vlan                  dev [VLAN name] dev [LINK MASTER] id [ID INTEGER] proto [PROTOCOL {802.1q|802.1ad}] Creates vlan netdev and network file\n"
...
static int cli_run(int argc, char *argv[]) {
        ...
        static const Ctl commands[] = {
                ...
                { "add-dhcpv4-server",            4,        WORD_ANY, false, ncm_link_add_dhcpv4_server },
