...
static int help(void) {
        printf("%s [OPTIONS...]\n\n"
               ...
               "  set-mtu                      [LINK] [MTU] Configures Link MTU\n"
               "  set-mac                      [LINK] [MAC] Configures Link MAC\n"
               "  set-link-mode                [LINK] [MODE {yes|no|on|off|1|0} ] Configures Link managed by networkd\n"
               ...
               "  add-link-address             [LINK] [ADDRESS] [PEER] ] Configures Link Address\n"
               "  delete-link-address          [LINK] Removes Address from Link\n"
               "  add-default-gateway          [LINK] [GW address] onlink [ONLINK {yes|no|on|off|1|0}] Configures Link Default Gateway\n"
               "  delete-gateway               [LINK] Removes Gateway from Link\n"
               "  add-route                    [LINK] [GW address] metric [METRIC { NUMBER }] Configures Link route\n"
               ...
               "  add-additional-gw            [LINK] address [ADDRESS] route [ROUTE address] gw [GW address] table [TABLE routing policy table NUMBER] Configure additional"
                                                      "\n\t\t\t\t      Configures gateway for another NIC with routing policy rules\n"
               ...
               "  add-dns                      [LINK | system] [ADDRESS] Configures Link DNS servers\n"
               "  add-domain                   [LINK | system] [DOMAIN] ConfiguresLink DOMAIN \n"
               "  show-domains                        Show DNS Server DOMAINS\n"
               "  revert-resolve-link          [LINK] Flushes all DNS server and Domain settings of the link\n"
               "  set-link-local-address       [LINK] [LinkLocalAddressing {yes|no|on|off|1|0}] Configures Link link-local address autoconfiguration\n"
               "  set-ipv4ll-route             [LINK] [IPv4LLRoute {yes|no|on|off|1|0}] Configures the route needed for non-IPv4LL hosts to communicate\n"
               "                                      with IPv4LL-only hosts\n"
               "  set-llmnr                    [LINK] [LLMNR {yes|no|on|off|1|0}] Configures Link Local Multicast Name Resolution\n"
               "  set-multicast-dns            [LINK] [MulticastDNS {yes|no|on|off|1|0}] Configures Link Multicast DNS\n"
               "  set-lldp                     [LINK] [LLDP {yes|no|on|off|1|0}] Configures Link Ethernet LLDP packet reception\n"
               "  set-emit-lldp                [LINK] [EmitLLDP {yes|no|on|off|1|0}] Configures Link Ethernet LLDP packet emission\n"
               "  set-ipforward                [LINK] [IPForward {yes|no|on|off|1|0}] Configures Link IP packet forwarding for the system\n"
               "  set-ipv6acceptra             [LINK] [IPv6AcceptRA {yes|no|on|off|1|0}] Configures Link IPv6 Router Advertisement (RA) reception support for the interface\n"
               "  set-ipv6mtu                  [LINK] [IPv6MTUBytes {yes|no|on|off|1|0}] Configures IPv6 maximum transmission unit (MTU)e\n"
               "  set-ipmasquerade             [LINK] [IPMasquerade {yes|no|on|off|1|0}] Configures IP masquerading for the network interface\n"
               "  set-dhcp4-use-dns            [LINK] [UseDNS {yes|no|on|off|1|0}] Configures Link DHCP4 Use DNS\n"
               "  set-dhcp4-use-domains        [LINK] [UseDomains {yes|no|on|off|1|0}] Configures Link DHCP4 Use DOMAINS\n"
               "  set-dhcp4-use-mtu            [LINK] [UseMTU {yes|no|on|off|1|0}] Configures Link DHCP4 Use MTU\n"
               "  set-dhcp4-use-ntp            [LINK] [UseNTP {yes|no|on|off|1|0}] Configures Link DHCP4 Use NTP\n"
               "  set-dhcp4-use-dns            [LINK] [UseDNS {yes|no|on|off|1|0}] Configures Link DHCP4 Use DNS\n"
               "  set-dhcp6-use-dns            [LINK] [UseDNS {yes|no|on|off|1|0}] Configures Link DHCP6 Use DNS\n"
               "  set-dhcp6-use-ntp            [LINK] [UseNTP {yes|no|on|off|1|0}] Configures Link DHCP6 Use NTP\n"
               ...
               "  add-ipv6ra                   [LINK] prefix [Prefix ADDRESS] pref-lifetime [PreferredLifetimeSec NUMBER] valid-lifetime [ValidLifetimeSec NUMBER]"
                                                      "\n\t\t\t\t      assign [Assign {yes|no|on|off|1|0}] managed [Managed {yes|no|on|off|1|0}]"
                                                      "\n\t\t\t\t      other [Other {yes|no|on|off|1|0}] dns [DNS ADDRESS] emit-dns [EmitDNS {yes|no|on|off|1|0}]"
                                                      "\n\t\t\t\t      domain [DOMAIN ADDRESS] emit-domain [EmitDOMAIN {yes|no|on|off|1|0}]"
